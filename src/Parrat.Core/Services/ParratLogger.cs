...
public class ParratLogger : IParratLogger
{
    ...
    public void Initialize()
    {
        ...
        _logPath = Path.Combine(logDir, $"parrat_{date}.log");

        try
        {
            _logMutex = new Mutex(false, @"Local\ParratLogMutex");
        }
        catch
        {
    ...
    public void Log(string level, string message, string? action = null, string? errorDetails = null)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                var fallback = Path.Combine(Path.GetTempPath(), "parrat_log_fallback.txt");
                try
                {
