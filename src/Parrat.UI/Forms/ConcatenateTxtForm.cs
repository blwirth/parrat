...
public class ConcatenateTxtForm : ParratFormBase
{
    ...
    {
        Text = "Concatenate TXT Files - Preview";
        Width = 1600;
        Height = 800;
        ...
        _tableFiles.Columns.Add("LineCount", typeof(int));
    ...
    private void LoadInitialFiles(string[] files)
    {
        ...
        foreach (var file in files)
        {
            try
            {
                var info = _concatenateService.GetTxtFileInfo(file);
                _fileInfos.Add(info);
            }
            catch (Exception ex)
            {
        ...
        {
            MessageBox.Show("No valid text files found.", "No Files",
    ...
    private void UpdateUI()
    {
        ...
        {
            if (item.TryGetValue("LineCount", out var lc) && lc is int count)
                totalLines += count;
        ...
        _lblSummary.Text = $"Files: {_fileInfos.Count} | Total Lines: {totalLines} | Total Size: {totalSizeMB} MB";
        ...
        foreach (var item in _fileInfos)
        {
            ...
            var lineCount = item.TryGetValue("LineCount", out var lc2) && lc2 is int c ? c : 0;
    ...
    private void BtnAddFiles_Click(object? sender, EventArgs e)
    {
        using var ofd = new OpenFileDialog
        {
            ...
            Title = "Select additional TXT files to add",
            Multiselect = true
        };
    ...
    private void BtnConcatenate_Click(object? sender, EventArgs e)
    {
        ...
        {
            Description = "Select output directory for concatenated TXT file"
        };
        ...
        var lblPrompt = new Label
        {
            ...
            Text = "Enter the output filename (without .txt extension):"
        };
        ...
        try
        {
            ...
            _concatenateService.WriteConcatenatedTxt(_fileInfos, outputPath);

            var openResult = MessageBox.Show(
                $"Concatenated TXT file saved to:\n{outputPath}\n\nTotal lines: {totalLines}\n\nOpen newly created file?",
                "Success",
                MessageBoxButtons.YesNo,
